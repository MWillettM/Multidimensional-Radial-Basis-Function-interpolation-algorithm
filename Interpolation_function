...
q is a variable controlling the size of sets for the cardinal functions generated as part of the algorithm. Usually we use 5<q<50 - q=30 is a safe value to use. The smaller the value of q, the quicker and dirtier the algorithm will be.error is the error you wish to be within.
'''
...
def FGP(data, values, c, q, error):
    ...
    while err > error:
        k += 1
        tau = np.sum(np.array([rf.step5(d[1], d[2], r) for d in data]), axis = 0)
        # tau = np.zeros(n)
